...
function localRecommend(input, userBar = [], drinks = [], context = {}, excludeIds = []) {
  ...
  if (top.length === 0) {
    const fallback = [...POPULAR_FALLBACK]
      ...
      .filter(Boolean);
    return { drinks: fallback, fallback: true };
  }
...
function extractProfile(normalized, current) {
  ...
  if (!p.flavor) {
    ...
    else if (/\b(citrico|azedo|limao|acido|sour|fresco|citrico)\b/.test(normalized)) p.flavor = 'citrico';
...
const QUESTIONS = {
  flavor: {
    key: 'flavor',
    text: 'Que tipo de sabor você curte mais? Amargo e complexo tipo Campari, doce e suave, cítrico com bastante limão, ou algo frutado e tropical?',
  },
  occasion: {
    key: 'occasion',
...
export default function BartenderIAScreen({ navigation }) {
  ...
  const handleSend = (text) => {
    ...
    setTimeout(() => {
      ...
      // Detect “show me another” request
      ...
      // Phase 1: gather info (max 2 questions, only before first recommendation)
      ...
      if (isAltRequest) {
        zeText     = excludeIds.length > 0 ? 'Mudando de rota. Aqui tem outras pedidas que funcionam:' : 'Olha, mais algumas opções:';
